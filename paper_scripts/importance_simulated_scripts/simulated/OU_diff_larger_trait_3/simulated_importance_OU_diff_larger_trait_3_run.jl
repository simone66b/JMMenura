...
##################################
@load "./simulated/OU_diff_larger_trait_3/OU_diff_larger_trait_3_para_ref_data.jld2" para_ref_data
#####################
# Set up parameters #
#####################
n = 4
...
function sim(N)
    ...
    max_reruns = 10000 # Change as needed
    reruns = 0
    for j in 1:N ## major loop for 5000 particles
        ...
        while !sol_stable && reruns < max_reruns
            ...
            mat_parameters = Dict(root_num => (alpha = alphasAll[j][n+1], mu = mat_mu, sigma = mat_sigma))
            trait_parameters = Dict(root_num => (alpha = alphasAll[j][1:n], mu = trait_mu, sigma = trait_sigma))
...
@save "./simulated/OU_diff_larger_trait_3/OU_diff_larger_trait_3_importance_result.jld2" tst unrun 