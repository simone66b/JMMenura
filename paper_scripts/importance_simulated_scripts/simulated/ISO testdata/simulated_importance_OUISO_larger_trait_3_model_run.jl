...
##################################
@load "/home/simoneb/Desktop/JMMenura/paper_scripts/importance_simulated_scripts/simulated/Test Data sets and trees/ISO_larger_trait_3_para_ref_data5D.jld2" para_ref_data_tree
...
mat_sigma = sqrt(2) ### mat_alpha = 0.0
mat_mu = copy(P0)
## mat_alpha = 1.0 
# data = para_ref_data
...
prior = Truncated(Normal(0.0, alphaPrior), 0.0, Inf)
priorvec = repeat([prior], n+1) # 4 traits and one for the matrix_diff
alphasAll = [rand.(priorvec) for i in 1:5000]## 8 + 2 draws from prior. 5000 particles
...
function sim(N, tree, data)
    ...
    for j in 1:N ## major loop for 5000 particles
        ...
        while !sol_stable && reruns < max_reruns
            if rerun
                push!(unrun, alphasAll[j])
                alphasAll[j] = rand.(priorvec)
            end
            mat_parameters = Dict(root_num => (sigma = mat_sigma, mu = mat_mu, alpha = alphasAll[j][n+1]))
            trait_parameters = Dict(root_num => (sigma = trait_sigma, mu = trait_mu, alpha = alphasAll[j][1:n]))
...
@save "/home/simoneb/Desktop/JMMenura/paper_scripts/importance_simulated_scripts/simulated/Test Data sets and trees/OUmodelled$i.ISOtestdata.jld2" tst unrun
