...
##################################
@load "./simulated/ISO_larger_trait_3/ISO_larger_trait_3_para_ref_data.jld2" para_ref_data
#####################
# Set up parameters #
#####################
n = 4
...
# OU simulation for traits
trait_alpha = repeat([0.0], n)
trait_mu = repeat([0.0], n)
trait_sigma = [2, 4, 6, 8]

# ISO simulation for matrix
...
priorvec = repeat([prior], n+2) # 4 traits and two for the matrix_diff
...
function sim(N)
    ...
    for j in 1:N ## major loop for 5000 particles
        ...
        while !sol_stable && reruns < max_reruns
            ...
            mat_parameters = Dict(root_num => (a = sigmasAll[j][n+1], b = sigmasAll[j][n+2]))
        ...
        Importances = [sigmasAll[j], push!(traitImportances, matImportances, matImportances)]
...
@save "./simulated/ISO_larger_trait_3/ISO_larger_trait_3_importance_result.jld2" tst unrun 