...
ther_ref_sim = menura_parameter_descend!(mat_parameters_true, trait_parameters_true, tree1, trait_evol_func, mat_evol_func, 0.0, ones(n)*10, P1, true)
...
thresholds = test_threshold(para_ref_data, tree1, para, ones(n)*10, P1, 1000, dt = 0.005, each = true)

threshold = sort(thresholds)[4]

@save "./threshold.jld2" threshold

sim_lower_diff_result = Vector{Any}()

@save "./s_big_lower_diff_result.jld2" sim_lower_diff_result
