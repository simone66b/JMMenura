...
BM_ref_sim = menura_parameter_descend!(mat_parameters_true, trait_parameters_true, tree1, trait_evol_func, mat_evol_func, 0.0, trait_mu, P0, true)

BM_ref_data = get_data(BM_ref_sim)

@save "big_BM_ref_data.jld2" BM_ref_data
...
OU_thresholds = [test_threshold(BM_ref_data, tree1, OU_para, trait_mu, mat_mu, n_threshold, each = true),
                test_threshold(BM_ref_data, tree1, BW_para, trait_mu, mat_mu, n_threshold, each = true), 
                test_threshold(BM_ref_data, tree1, Iso_para, trait_mu, mat_mu, n_threshold, each = true)]

threshold = minimum([sort(model_thresholds)[4] for model_thresholds in OU_thresholds])

@save "./threshold.jld2" threshold

s_model_big_BM_result = Vector{Any}()

@save "./s_model_big_BM_result.jld2" s_model_big_BM_result
