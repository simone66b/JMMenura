...
function recurse_menura_each!(tree, node, t0 , x0, trait_drift, trait_diff, matrix_drift, matrix_func)
    if ismissing(node.inbound) ## the root node, to get started
        ...
        evol = trait_diffusion_each(ancestor.data["trace"][end],
                         ...
                         ancestor.data["parameters"], 
                         node.data["matrixes"], trait_drift, trait_diff)
        node.data["trace"] = evol.u
        node.data["timebase"] = evol.t
        
    end
    if !isleaf(tree, node)
        recurse_menura_each!(tree, node.other[1].inout[2], t0 , x0, trait_drift, trait_diff, matrix_drift, matrix_func)
        recurse_menura_each!(tree, node.other[2].inout[2], t0 , x0, trait_drift, trait_diff, matrix_drift, matrix_func)
    end
end
...
"""
function menura_each!(tree, x0, trait_drift, trait_diff, matrix_drift, matrix_func)
    root = getroot(tree)
...
function menura_sim_each(alpha, sigma, mu, cov_mat, tree, matrix_func; a = nothing, b = nothing, x0 = nothing, mat_alpha = nothing
    ...
    menura_each!(tree, x0, trait_drift, trait_diff, matrix_drift, matrix_func)
