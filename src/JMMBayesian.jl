...
function sufficient_distance(var_num, leaf_num; err_thres = 10^-14)
    function trait_mat_dist(data1, data2)
        data1 = reshape(data1, var_num, (var_num+1)*leaf_num)
        data2 = reshape(data2, var_num, (var_num+1)*leaf_num)
...
function sufficient_distance_v3(var_num, leaf_num; err_thres = 10^-14)
    function trait_mat_dist(data1, data2)
        data1 = reshape(data1, var_num, (var_num+1)*leaf_num)
        data2 = reshape(data2, var_num, (var_num+1)*leaf_num)
        ...
        trait_diff =  abs(sum(abs.(mean(traits1) - mean(traits2))))
        hermi_matrices1, hermi_matrices2 = Hermitian.(mats1), Hermitian.(mats2)
...
function sufficient_distance_v4(var_num, leaf_num, tree; err_thres = 10^-14)
    heights = heightstoroot(tree)
    height_mat = repeat(heights, 1, leaf_num)
    inv_vcv = inv((height_mat + height_mat' - distances(tree))./2)

    function trait_mat_dist(data1, data2)
        data1 = reshape(data1, var_num, (var_num+1)*leaf_num)
        data2 = reshape(data2, var_num, (var_num+1)*leaf_num)

        traits1 = data1[:,1:leaf_num]
        traits2 = data2[:,1:leaf_num]

        mats1 = [data1[:,(leaf_num + var_num*(i-1)+1):(leaf_num + var_num*(i))] for i in 1:leaf_num]
        mats2 = [data2[:,(leaf_num + var_num*(i-1)+1):(leaf_num + var_num*(i))] for i in 1:leaf_num]

        diff1 = traits1 .- mean(traits1, dims = 2)
        diff2 = traits2 .- mean(traits2, dims = 2)

        trait_diff =  abs(sum(abs.(mean(traits1) - mean(traits2))) + 
                                abs(tr(diff1 * inv_vcv * diff1') - tr(diff2 * inv_vcv * diff2')))
        hermi_matrices1, hermi_matrices2 = Hermitian.(mats1), Hermitian.(mats2)
