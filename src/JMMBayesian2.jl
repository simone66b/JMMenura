...
function trait_mat_importance(var_num, leaf_num; err_thres = 10^-14)
    function trait_mat_imp(data1, data2)
        ...
        trait_importance =  1.0 ./ ((traits1 - traits2) .^ 2.0) ## unnormalised
        ...
        matrix_importance = [1.0 / ((PosDefManifold.distance(Fisher, hermi_matrices1[i], hermi_matrices2[i]))^2.0) for i in 1:leaf_num]
        ## matrix_diff = mean([sqrt(sum(log.(max.(eigvals(hermi_matrices1[i], hermi_matrices2[i]), 0)).^2)) for i in 1:leaf_num]) # Fisher Rao metric
        return [trait_importance matrix_importance]
    end
end


function mat_importance(var_num, leaf_num; err_thres = 10^-14)
    function mat_imp(data1, data2)
...
function trait_importance(var_num, leaf_num; err_thres = 10^-14)
    function trait_dist(data1, data2)
        ...
        trait_importance =  1.0 ./ ((traits1 - traits2) .^ 2.0)
...
"""
function menura_bayesian(reference_data, tree, JMMpara::JMMABCparameters, trait0, mat0, threshold, n_particles; max_iter = n_particles, t0 = 0.0, each = false, 
    dt = 0.01, distance_function = trait_mat_distance(JMMpara.size,nleaves(tree)), summary_function = get_data, verbose = true)
    
    preallocate_tree!(tree, dt, JMMpara.size)

    bayesian_menura! = create_bayesian_sim(tree, JMMpara, trait0, mat0, t0 = t0, each = each, dt = dt, 
    summary_function = summary_function, verbose = verbose)

    SimulatedABCRejection(reference_data, bayesian_menura!, get_priors(JMMpara), [Inf], n_particles, 
    max_iter = max_iter, write_progress = true, distance_function = dfunc)
end
dfunc(distance) = 1.0
